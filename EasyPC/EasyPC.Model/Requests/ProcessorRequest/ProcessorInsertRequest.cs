using System.ComponentModel.DataAnnotations;

namespace EasyPC.Model.Requests.ProcessorRequests;

public class ProcessorInsertRequest
{
    [Required(ErrorMessage = "Processor name is required")]
    [MinLength(2, ErrorMessage = "Name must have at least 2 characters")]
    [MaxLength(100, ErrorMessage = "Name cannot exceed 100 characters")]
    public required string Name { get; set; }

    [Required(ErrorMessage = "Socket is required")]
    [MinLength(2, ErrorMessage = "Socket must have at least 2 characters")]
    [MaxLength(50, ErrorMessage = "Socket cannot exceed 50 characters")]
    public required string Socket { get; set; }

    [Required(ErrorMessage = "Price is required")]
    [Range(1, int.MaxValue, ErrorMessage = "Price must be greater than 0")]
    public required int Price { get; set; }

    [Required(ErrorMessage = "Core count is required")]
    [Range(1, 128, ErrorMessage = "Core count must be between 1 and 128")]
    public required int CoreCount { get; set; }

    [Required(ErrorMessage = "Thread count is required")]
    [Range(1, 256, ErrorMessage = "Thread count must be between 1 and 256")]
    public required int ThreadCount { get; set; }

    [Required(ErrorMessage = "Manufacturer is required")]
    [Range(1, int.MaxValue, ErrorMessage = "You must select a manufacturer")]
    public required int ManufacturerId { get; set; }
}
