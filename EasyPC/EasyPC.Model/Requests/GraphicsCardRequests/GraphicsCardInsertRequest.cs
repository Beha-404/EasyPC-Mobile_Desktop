using System.ComponentModel.DataAnnotations;

namespace EasyPC.Model.Requests.GraphicsCardRequests
{
    public class GraphicsCardInsertRequest
    {
        [Required(ErrorMessage = "Graphics card name is required")]
        [MinLength(2, ErrorMessage = "Name must have at least 2 characters")]
        [MaxLength(100, ErrorMessage = "Name cannot exceed 100 characters")]
        public required string Name { get; set; }

        [Required(ErrorMessage = "VRAM is required")]
        [MinLength(2, ErrorMessage = "VRAM must have at least 2 characters")]
        [MaxLength(50, ErrorMessage = "VRAM cannot exceed 50 characters")]
        public required string VRAM { get; set; }

        [Required(ErrorMessage = "Price is required")]
        [Range(1, int.MaxValue, ErrorMessage = "Price must be greater than 0")]
        public required int Price { get; set; }

        [Required(ErrorMessage = "Manufacturer is required")]
        [Range(1, int.MaxValue, ErrorMessage = "You must select a manufacturer")]
        public required int ManufacturerId { get; set; }
    }
}
