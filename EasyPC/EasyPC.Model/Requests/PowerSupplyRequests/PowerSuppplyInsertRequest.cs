using System.ComponentModel.DataAnnotations;

namespace EasyPC.Model.Requests.PowerSupplyRequests;

public class PowerSupplyInsertRequest
{
    [Required(ErrorMessage = "Power supply name is required")]
    [MinLength(2, ErrorMessage = "Name must have at least 2 characters")]
    [MaxLength(100, ErrorMessage = "Name cannot exceed 100 characters")]
    public required string Name { get; set; }

    [Required(ErrorMessage = "Power is required")]
    [MinLength(2, ErrorMessage = "Power must have at least 2 characters")]
    [MaxLength(50, ErrorMessage = "Power cannot exceed 50 characters")]
    public required string Power { get; set; }

    [Required(ErrorMessage = "Price is required")]
    [Range(1, int.MaxValue, ErrorMessage = "Price must be greater than 0")]
    public required int Price { get; set; }

    [Required(ErrorMessage = "Manufacturer is required")]
    [Range(1, int.MaxValue, ErrorMessage = "You must select a manufacturer")]
    public required int ManufacturerId { get; set; }
}
