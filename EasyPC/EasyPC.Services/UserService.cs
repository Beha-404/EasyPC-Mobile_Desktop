...
public class UserService(DatabaseContext context, IMapper mapper) : IUserService
{
    ...
    public Model.User? Update(int id, UserUpdateRequest updateRequest)
    {
        ...
        cfg.ForType<UserUpdateRequest, User>()
    ...
    public Model.User? UpdateRole(UpdateRoleRequest updateRoleRequest)
    {
        ...
        entity.Role = (UserRole)updateRoleRequest.NewRole;
        _context.SaveChanges();
        return _mapper.Map<Model.User>(entity);
    }

    protected IQueryable<User> ApplyFilter(IQueryable<User> query, UserSearchObject? searchObject)
    {
