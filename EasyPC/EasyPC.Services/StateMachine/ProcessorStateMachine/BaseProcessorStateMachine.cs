...
public class BaseProcessorStateMachine(DatabaseContext context, IMapper mapper, IServiceProvider serviceProvider) : BaseStateMachine<Model.Processor, ProcessorInsertRequest, ProcessorUpdateRequest, Processor>(context, mapper, serviceProvider)
{
    public override IBaseStateMachine<Model.Processor, ProcessorInsertRequest, ProcessorUpdateRequest, Processor> NextState(string state)
    {
        return state switch
        {
